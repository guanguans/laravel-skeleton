...
class HealthCheckCommand extends Command
{
    ...
    protected function checkDatabase($connection = null): HealthCheckStateEnum
    {
        try {
            ...
            return tap(HealthCheckStateEnum::FAILING(), static function (HealthCheckStateEnum $state) use ($throwable) {
    ...
    protected function checkSqlSafeUpdates(): HealthCheckStateEnum
    {
        if (config('database.default') !== 'mysql') {
            return tap(HealthCheckStateEnum::WARNING(), static function (HealthCheckStateEnum $state) {
        ...
        if (! Str::of($sqlSafeUpdates->Value)->lower()->is('on')) {
            return tap(HealthCheckStateEnum::FAILING(), static function (HealthCheckStateEnum $state) {
    ...
    protected function checkSqlMode($checkedSqlModes = 'strict_all_tables'): HealthCheckStateEnum
    {
        ...
        if ($diffSqlModes->isNotEmpty()) {
            return tap(HealthCheckStateEnum::FAILING(), static function (HealthCheckStateEnum $state) use ($diffSqlModes) {
    ...
    protected function checkTimeZone(): HealthCheckStateEnum
    {
        ...
        if ($dbDateTime !== $appDateTime) {
            return tap(HealthCheckStateEnum::FAILING(), static function (HealthCheckStateEnum $state) use ($appTimezone, $dbTimeZone) {
    ...
    protected function checkPing(?string $url = null): HealthCheckStateEnum
    {
        ...
        if ($response->failed()) {
            return tap(HealthCheckStateEnum::FAILING(), static function (HealthCheckStateEnum $state) use ($response) {
    ...
    protected function checkPhpExtensions(): HealthCheckStateEnum
    {
        ...
        if ($missingExtensions->isNotEmpty()) {
            return tap(HealthCheckStateEnum::FAILING(), static function (HealthCheckStateEnum $state) use ($missingExtensions) {
    ...
    protected function checkDiskSpace(): HealthCheckStateEnum
    {
        ...
        if ($diskSpace < 100) {
            return tap(HealthCheckStateEnum::FAILING(), static function (HealthCheckStateEnum $state) use ($diskSpace) {
        ...
        if ($diskSpace < 1) {
            return tap(HealthCheckStateEnum::WARNING(), static function (HealthCheckStateEnum $state) use ($diskSpace) {
    ...
    protected function checkMemoryLimit(int $limit = 128): HealthCheckStateEnum
    {
        ...
        if ($localValue < $limit) {
            return tap(HealthCheckStateEnum::FAILING(), static function (HealthCheckStateEnum $state) use ($limit, $localValue) {
